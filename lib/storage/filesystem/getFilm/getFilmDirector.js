...
function getFilmDirector(director) {
    try{
        ...
        if(!empty){
            ...
            if(response != ''){
                return {'success': true, 'message': 'Some films were returned by its director', 'content': response};
            }else{
                return {'success': false, 'message': 'Film with the director passed as argument does not exist'};
            }
        }else{

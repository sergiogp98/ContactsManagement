...
describe('Modify film', function(){
    it('Ckeck if a film title is modified', function(){
        var filmTest = film;
        filmTest.title = 'Batman';
        addModule.addFilm(film);
        assert.equal(modifyModule.modifyFilmTitle(film, 'Batman'), 'Film title modified correctly');
        deleteModule.deleteFilm(filmTest);
    });
    ...
    it('Check if a film year is modified', function(){
        ...
        filmTest.year = 2020;
        addModule.addFilm(film);
        assert.equal(modifyModule.modifyFilmYear(film, 2020), 'Film year modified correctly');
        deleteModule.deleteFilm(filmTest);
    });
    ...
    it('Check if a film length is modified', function(){
        ...
        filmTest.length = 150;
        addModule.addFilm(film);
        assert.equal(modifyModule.modifyFilmLength(film, 150), 'Film length modified correctly');
        deleteModule.deleteFilm(filmTest);
    });

    it('Check if a film genre is modified', function(){
        var filmTest = film;
        filmTest.genre = 'Science fiction';
        addModule.addFilm(film);
        assert.equal(modifyModule.modifyFilmGenre(film, 'Science fiction'), 'Film genre modified correctly');
        deleteModule.deleteFilm(filmTest);
    });

    it('Check if a film director is modified', function(){
        var filmTest = film;
        filmTest.director = 'Quentin Tarantino';
        addModule.addFilm(film);
        assert.equal(modifyModule.modifyFilmDirector(film, 'Quentin Tarantino'), 'Film director modified correctly');
        deleteModule.deleteFilm(filmTest);
    });
});
*/